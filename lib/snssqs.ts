...
export class SNSSQSStack extends cdk.Stack {
    constructor(scope: cdk.Construct, id: string, props?: cdk.StackProps) {
        ...
        const dbendpoint = new cdk.CfnParameter(this, 'dbendpoint',{
            type: "String",
            default: "default"
        })
                ...
                statements: [
                    new iam.PolicyStatement({
                        ...
                        conditions: [
                            {
                                StringEquals: {
                                    "aws:SourceArn": UploadSNSTopic.topicArn
                                }
                            }]
                    })
                ]
        ...
        const UploadTopicPolicy = new sns.CfnTopicPolicy(this, 'UploadTopicPolicy', {
            policyDocument: new iam.PolicyDocument({
                statements: [
                    new iam.PolicyStatement({
                        effect: iam.Effect.ALLOW,
        ...
        const LabelsLambda = new lambda.Function(this, 'LabelsLambda', {
            ...
            code: new lambda.InlineCode(fs.readFileSync('./lib/lambda-image-labels/lambda_function.js', { encoding: 'utf-8' })),  // code loaded from the "lambda" directory
            ...
            environment: {
                ...
                'DATABASE_PASSWORD': DBPassword.valueAsString,
        ...
        const LambdaSubscription = new sns.Subscription(this, 'LambdaSubscription', {
            endpoint: LabelsLambda.functionArn,
